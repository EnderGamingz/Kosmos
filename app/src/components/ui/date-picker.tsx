...
function Calendar({
  ...
  const MONTHS = React.useMemo(() => {
    let locale: Pick<Locale, 'options' | 'localize' | 'formatLong'> =
      APP_TIME_LOCALE;
  ...
  const matcher: Matcher | undefined = disableFuture
    ? { after: today }
    : disablePast
      ? { before: today }
      : undefined;
  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        day: 'mr-1 last:mr-0 h-9 w-9 text-center text-sm p-0 relative rounded-md [&:has([aria-selected].day-outside)]:bg-accent/50 [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20 rounded-1',
        day_button: cn(
...
type DateTimePickerProps = {
  ...
  selected?: unknown;
  onSelect?: (date: unknown | undefined) => void;
} & Pick<
  CalendarProps,
      ...
      <Popover>
        ...
        <PopoverContent className='w-auto p-0'>
          <Calendar
            ...
            mode={mode ?? 'single'}
            // @ts-expect-error newDate only works with single mode
